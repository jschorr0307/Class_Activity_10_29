...
#keep everything on the left but only keep what matches on the right
#band is the left data set, intrument is the right
band %>% right_join(instrument, by = "name")
#keep everything on the right and add the stuff that matches on the left
#band is on the left and instrument is on the right
...
#only keep colums from first data set and only keep what matches in the rows of both
band %>% anti_join(instrument, by = "name")
#only keep things that did NOT match in the table
```
